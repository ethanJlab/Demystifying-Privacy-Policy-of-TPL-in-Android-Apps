...
def processFile(filename, outputDir=None):
    try:
        outputfilename = outputDir + '/' + '{}.txt'.format(filename.split('/')[-2])
...
class Preprocessor:
    ...
    def parse(self):
        ...
        def handleListItemText(prependText, listItemText):
            ...
            prependText = re.sub(r'\s*:\s*$', u'',
                                 prependText) if u'following' not in prependText.lower() else prependText
        ...
        def handlePlaintextMultilineList(outputDoc, pars, index, prependText):
            ...
            def combineText(prependText, appendText):
                ptext = re.sub(r'\s*:\s*$', u'', stripPlaintextListFormatters(
                    prependText.strip())) if u'following' not in prependText.lower() else stripPlaintextListFormatters(
                    prependText.strip())
            ...
            def checkLIType(text, expectedToken=None):
                ...
                if sresult:
                    ...
                    # splt[-1] = str(int(splt[-1]) + 1).__unicode__()
                    splt[-1] = str(int(splt[-1]) + 1)
                ...
                if sresult:
                    if expectedToken is not None and expectedToken != ';':
                        return defaultFResult
                    return (
                        True, u';', re.search(r';\s+(and|or)\s*$', text, re.IGNORECASE))  # All items must with with ;
        ...
        def processMarkdown(outputDoc, pars, index=0, prependText=None):
            ...
            def ensureNotTitle(text):
                if len(text.strip()) == 0:
                    return False
                ncap = [1 if t[0].isupper() else 0 for t in text.strip().split(u' ') if
                        len(t) > 0 and t not in [u'a', u'an', u'the', u'and', u'but', u'for', u'on', u'to', u'at',
                                                 u'by', u'from', u'in', u'into', u'like', u'of', u'near', u'off',
                                                 u'onto', u'out', u'over', u'past', u'up', u'upon', u'with', u'within',
                                                 u'without']]
    ...
    def process(self):
        ...
        def isSkippableElement(element):
            def isSpanScreenReaderOnlyElement(element):
                return element.name == 'span' and element.get('class') is not None and u'sr-only' in element.get(
                    'class')
            ...
            def isNavigationLink(element):
                ...
                #     unicode(element), flags=re.IGNORECASE)
                return element.name == 'a' and re.search(
                    r'^<a\s.*>(Learn\sMore|(Back|Return)([\-\s]to[\-\s]Top)?|Skip\sto\s(content|navigation))</a>$',
                    element.__unicode__(), flags=re.IGNORECASE)
        ...
        def isPopupElement(element):
            return element.name in [u'span', u'div'] and element.get(
            ...
            if isIteratable(element):
                ...
                for child in element.childGenerator():
                    ...
                    if name is not None:  # Container
                        ...
                        child.replaceWith(u''.join([BEGIN_TAG, child, u'&lt;/LISTITEM&gt;']))
...
class TextPostProcessor:
    ...
    def handleInlineList(self, text):
        ...
        if re.search(r':\s*(\(\s*([0-9][0-9]?|[A-Za-z]+)\s*\)|([0-9][0-9]?|[A-Za-z])(\s*\)|\.|\\\.))\s*', text):
            text = re.sub(r'\s+(\(\s*([0-9][0-9]?|[A-Za-z]+)\s*\)|([0-9][0-9]?|[A-Za-z])(\s*\)|\.|\\\.))\s*', u' ',
                          text)
        return text
    ...
    def postProcess(self):
        def stripPlaintextListFormatters(text):
            ...
            text = re.sub(r'([;\.\?:!]\s+|^)([0-9][0-9]?|[A-Za-z])(\s*\)|\.|\\\.)\s+',
                          lambda m: re.sub(r'(\s+|^)([0-9][0-9]?|[A-Za-z])(\s*\)|\.|\\\.)', u' ', m.group(0)), text)
...
def incrementListItemCallback(m):
    if re.search(r'^[0-9]+$', m.group(0)):  # Digit
        ...
        return u'a' if m.group(0) == u'z' else str(chr(ord(m.group(0)) + 1))
    elif re.search(r'^[A-Z]$', m.group(0)):
        # return u'A' if m.group(0) == u'Z' else unicode(str(chr(ord(m.group(0)) + 1)))
        return u'A' if m.group(0) == u'Z' else str(chr(ord(m.group(0)) + 1))

        # return u'A' if m.group(0) == u'Z' else str(chr(ord(m.group(0)) + 1)).__unicode__()
    else:
        print
        'Error', m
    return m
